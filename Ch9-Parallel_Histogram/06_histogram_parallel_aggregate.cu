...
#define COARSE_LEVEL 4

__global__ void histogram_parallel_coarse_aggregate(char *sentence_data_device, unsigned int sentence_len, unsigned int *histogram_device) 
{
    ...
    // Book keeping
    unsigned int accumulator = 0; // Variable to aggregate the contiguous values (if present in the data)
    int prev_bin = -1; // Which bin did last value belong to

    // Spreading the threads such that there are gaps in between to be filled by loop iteration
    for (unsigned int i_coarse = i; i_coarse < sentence_len; i_coarse+=blockDim.x*gridDim.x)
    {
        int alphabet_pos = sentence_data_device[i_coarse] - 'a'; // Position of the alphabet
        if (alphabet_pos >= 0 && alphabet_pos < 26) // Store if it is lowercase
        {
            int this_bin = alphabet_pos/4; // Which bin this element belongs to
            if (this_bin == prev_bin) // If the element is the same as before 
            {
                accumulator += 1;
            }
            else
            {
                if (accumulator > 0)
                {
                    atomicAdd(&sh_histogram[prev_bin], accumulator); // Storing the accumulated values in shared memory histogram
                }
                accumulator = 1;
                prev_bin = this_bin;
            }
    ...
    if (accumulator > 0) // Inside the loop only prev_bin is stored. There is a delay so need to make sure that everything is stored
    {
        atomicAdd(&sh_histogram[prev_bin], accumulator); // Storing in shared memory histogram
    }
...
int main(int argc, char const *argv[])
{
    ...
    unsigned int num_blocks = ceil(sentence_len/(float)(num_threads_per_block*COARSE_LEVEL)); // Total number of blocks
    ...
    // Kernel execution
    cudaEventRecord(beg);
    histogram_parallel_coarse_aggregate<<<num_blocks, num_threads_per_block>>>(sentence_data_device, sentence_len, histogram_device);
    cudaEventRecord(end);
