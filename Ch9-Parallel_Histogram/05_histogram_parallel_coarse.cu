...
__global__ void histogram_parallel_coarse_contiguous(char *sentence_data_device, unsigned int sentence_len, unsigned int *histogram_device) 
{
    ...
    for (unsigned int i_coarse = i*COARSE_LEVEL; i_coarse < min(i*COARSE_LEVEL+COARSE_LEVEL, sentence_len); i_coarse++)
    {
...
__global__ void histogram_parallel_coarse_interleaved(char *sentence_data_device, unsigned int sentence_len, unsigned int *histogram_device) 
{
    unsigned int i = blockIdx.x*blockDim.x + threadIdx.x; // Global thread index
...
int main(int argc, char const *argv[])
{
    ...
    // Kernel execution
    cudaEventRecord(beg);
    histogram_parallel_coarse_contiguous<<<num_blocks, num_threads_per_block>>>(sentence_data_device, sentence_len, histogram_device);
    cudaEventRecord(end);
