...
__global__ void histogram_parallel_atomic(char *sentence_data_device, unsigned int sentence_len, unsigned int *histogram_device) 
{
    unsigned int i = blockIdx.x*blockDim.x + threadIdx.x; // Thread index
    ...
    if (i < sentence_len)
    {
        ...
        if (alphabet_pos >= 0 && alphabet_pos < 26) // Store if it is lowercase
            atomicAdd(&histogram_device[alphabet_pos/4], 1);
    }
}
...
int main(int argc, char const *argv[])
{
    ...
    // Kernel execution
    cudaEventRecord(beg);
    histogram_parallel_atomic<<<ceil(sentence_len/256.0), 256>>>(sentence_data_device, sentence_len, histogram_device);
    cudaEventRecord(end);
